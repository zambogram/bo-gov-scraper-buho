"""
Scraper for Autoridad de Supervisión del Sistema Financiero (ASFI)
"""
...
class ASFIScraper(BaseScraper):
    ...
    def scrape(self, limit: Optional[int] = None, only_new: bool = False) -> Dict:
        """Scrape ASFI website for financial regulations"""
        ...
        # Simulated scraping
        sample_urls = [
            f'{self.base_url}/normativa/resolucion-asfi-{str(i).zfill(4)}-2024.html'
            for i in range(1, (limit or 10) + 1)
        ]

        for i, url in enumerate(sample_urls, 1):
            try:
                doc_id = f'asfi-{str(i).zfill(6)}'
                title = f'Resolución ASFI {str(i).zfill(4)}/2024'
                content = f'Contenido simulado de la resolución ASFI {i}...'
