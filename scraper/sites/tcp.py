...
class TCPScraper(BaseSiteScraper):
    ...
    def scrape(self, limit: Optional[int] = None, solo_nuevos: bool = True) -> List[DocumentoScrapeado]:
        """Scrapea sentencias del TCP."""
        self.logger.info(f"Iniciando scraping de TCP (modo_demo={self.modo_demo})")
    ...
    def _scrape_real(self, limit: Optional[int] = None) -> List[DocumentoScrapeado]:
        ...
        try:
            ...
            items = soup.select(self.site_config.selectores.get('tabla_resultados', 'div.sentencia-item'))
            ...
            for i, item in enumerate(items):
                ...
                try:
                    numero_elem = item.select_one(self.site_config.selectores.get('numero_sentencia', 'h4.numero'))
                    fecha_elem = item.select_one(self.site_config.selectores.get('fecha', 'span.fecha-publicacion'))
                    tipo_elem = item.select_one(self.site_config.selectores.get('tipo', 'span.tipo-sentencia'))
                    pdf_elem = item.select_one(self.site_config.selectores.get('enlace_pdf', 'a.btn-descargar'))
                    ...
                    tipo = tipo_elem.get_text(strip=True) if tipo_elem else "Sentencia Constitucional"
    ...
    def _scrape_modo_demo(self, limit: Optional[int] = None) -> List[DocumentoScrapeado]:
        """Genera datos de demostración para TCP."""
        self.logger.info("Generando datos de demostración para TCP")

        documentos = []
        num_docs = limit if limit else 10

        for i in range(num_docs):
            tipo = self.TIPOS_SENTENCIA[i % len(self.TIPOS_SENTENCIA)]
            materia = choice(self.MATERIAS)
            numero = f"{100 + i}/2024"

            dias_atras = randint(0, 90)
            fecha = (datetime.now() - timedelta(days=dias_atras)).strftime("%Y-%m-%d")

            titulo = f"{tipo} {numero} - {materia}"
            ...
            documento = DocumentoScrapeado(
                ...
                url_pdf=f"{self.site_config.url_base}/sentencias/SC_{numero.replace('/', '_')}.pdf",
                metadata_extra={'materia': materia, 'modo': 'demo'}
            )
