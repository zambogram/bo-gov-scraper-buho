"""Scraper ATT - SCRAPING REAL de resoluciones administrativas"""
...
class ATTScraper(BaseScraper):
    ...
    def listar_documentos(self, limite: Optional[int] = None, modo: str = "delta", pagina: int = 1) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            if docs:
                logger.info(f"✓ Scraping REAL: {len(docs)} resoluciones ATT")
                return docs
        except Exception as e:
            logger.warning(f"⚠ Scraping falló: {e}")
        logger.error("✗ No se pudo obtener datos de ATT")
        return []
    ...
    def _extraer_doc(self, enlace) -> Optional[Dict[str, Any]]:
        ...
        if 'reglamento' in texto.lower():
            tipo = 'Reglamento'
        elif 'norma técnica' in texto.lower():
            tipo = 'Norma Técnica'
        id_doc = f"att_ra_{numero.replace('/', '_') if numero else hash(url_pdf) % 100000}"
        return {
            'id_documento': id_doc,
            'tipo_documento': tipo,
            ...
            'anio': año,
            'fecha': f"{año}-01-01",
            'titulo': texto[:200] if texto else f"{tipo} ATT {numero}",
            'url': url_pdf,
            'sumilla': f"{tipo} - Telecomunicaciones y Transportes",
            'metadata_extra': {'entidad': 'ATT', 'area': 'administrativo', 'metodo_scraping': 'real'}
        }
