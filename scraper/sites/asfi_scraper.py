"""Scraper ASFI - SCRAPING REAL de resoluciones administrativas"""
...
class ASFIScraper(BaseScraper):
    ...
    def listar_documentos(self, limite: Optional[int] = None, modo: str = "delta", pagina: int = 1) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            if docs:
                logger.info(f"✓ Scraping REAL: {len(docs)} resoluciones ASFI")
                return docs
        except Exception as e:
            logger.warning(f"⚠ Scraping falló: {e}")
        logger.error("✗ No se pudo obtener datos de ASFI")
        return []
    ...
    def _extraer_doc(self, enlace) -> Optional[Dict[str, Any]]:
        ...
        if 'circular' in texto.lower():
            tipo = 'Circular'
        elif 'reglamento' in texto.lower():
            tipo = 'Reglamento'
        id_doc = f"asfi_ra_{numero.replace('/', '_') if numero else hash(url_pdf) % 100000}"
        return {
            'id_documento': id_doc,
            'tipo_documento': tipo,
            ...
            'anio': año,
            'fecha': f"{año}-01-01",
            'titulo': texto[:200] if texto else f"{tipo} ASFI {numero}",
            'url': url_pdf,
            'sumilla': f"{tipo} - Supervisión del sistema financiero",
            'metadata_extra': {'entidad': 'ASFI', 'area': 'financiero', 'metodo_scraping': 'real'}
        }
